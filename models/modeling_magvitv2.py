...
class LFQuantizer(nn.Module):
    ...
    def get_codebook_entry_gs(self, indices, shape=None):
        if shape is None:
            h, w = int(math.sqrt(indices.shape[-2])), int(math.sqrt(indices.shape[-2]))
        else:
            h, w = shape
        # import pdb; pdb.set_trace()
        b, _, _ = indices.shape
        ...
        # z_q = self.embedding[indices]
        z_q = torch.matmul(indices, self.embedding)  # shape: [batch, tokens, c]
        z_q = z_q.view(b, h, w, -1)
        # reshape back to match original input shape
        z_q = z_q.permute(0, 3, 1, 2).contiguous()
...
class MAGVITv2(ModelMixin, ConfigMixin):
    ...
    def decode_code_gs(self, codebook_indices, shape=None):
        z_q = self.quantize.get_codebook_entry_gs(codebook_indices, shape=shape)
