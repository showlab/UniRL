...
class Showo(ModelMixin, ConfigMixin):
    ...
    def forward(
        ...
        if input_embeddings is None:
            logits = self.showo(input_ids=input_ids, attention_mask=attention_mask)['logits'] if token_mask is None else self.showo(input_ids=input_ids, attention_mask=attention_mask, token_mask = token_mask)['logits']
        else:
            logits = self.showo(inputs_embeds=input_embeddings, attention_mask=attention_mask)['logits'] if token_mask is None else self.showo(input_ids=input_ids, attention_mask=attention_mask, token_mask = token_mask)['logits']
    ...
    def t2i_generate(
        ...
        for step in range(timesteps):
            if uncond_input_ids is not None and guidance_scale > 0:
                ...
                cond_logits, uncond_logits = self(model_input, attention_mask=attention_mask,token_mask=token_mask).chunk(2)
                ...
                logits = self(input_ids, attention_mask=attention_mask,token_mask=token_mask)
                logits = logits[:, -(num_vq_tokens + 1):-1, config.model.showo.llm_vocab_size + num_new_special_tokens:-1]
