...
def run_example(question,gt,llm_output):
    prompt = f"Please help to judge the two answers same or not.The question is {question} One is the standard answer, another is model's output. The standard answer is {gt} and the model's output is {llm_output}. Please only use one word to answer me same or not same"
    ...
    model_inputs = qwen_tokenizer([text], return_tensors="pt").to(model.device)
...
if __name__ == '__main__':
    ...
    if config.experiment.use_lora:
                    ...
                    and all(
                        [
                            ...
                            for x in [
                                ...
                                "mm_projector",
                                # "text_hidden_fcs",
                                # "t_embedder", "y_embedder", "x_dim_up"
        ...
        lora_target_modules = find_linear_layers(
            model.showo, "q_proj,v_proj".split(",")
        )
    ...
    with open("/mnt/bn/vgfm2/test_dit/weijia/outputs16/januspro3.jsonl") as  f:
        validation_prompts = f.read().splitlines()

    with open("/mnt/bn/vgfm2/test_dit/weijia/code_cycle/validation_prompts/evaluation_metadata_qa.jsonl") as  f:
        validation_prompts_original = f.read().splitlines()
    ...
    for step in tqdm(range(0, len(validation_prompts))):
        ...
        data['prompt'] = prompts
        # prompts = ['a photo of a towel']
        answer =  data['answer']
        image_path = data['image_path']
        sample_count = image_path.split("/")[-3]+"_"+image_path.split("/")[-2]+"_"+image_path.split("/")[-1]

        top_k = 1
        total_pil_images = []
        
        if data['geneval_correct']== False:
            continue
        # import pdb; pdb.set_trace()
        ...
        # import pdb; pdb.set_trace()
        image_result = {}
        # image_path = "/mnt/bn/vgfm2/test_dit/weijia/outputs17/output_grpo_beta_0_2_all_kinds_5k/00138/samples/00000.png"
        # import pdb; pdb.set_trace()
        image_ori = Image.open(image_path).convert("RGB")
        image = image_transform(image_ori, resolution=config.dataset.params.resolution).to(device)
        image = image.unsqueeze(0)
        image_tokens = vq_model.get_code(image)
        # import pdb; pdb.set_trace()

        text = mmu_generate(model, uni_prompting, question, device, top_k, image_tokens)
        result = run_example(
            question =  question,
            gt = answer,
            llm_output = text
        )
        image_result['tag'] = data['tag']
        image_result['image_path'] = data['image_path']
        image_result['answer'] = answer
        image_result['question'] = question
        ...
        if result.lower()=="same": 
            ...
            # text = mmu_generate(model, uni_prompting, question, device, top_k, gen_token_ids)
            # import pdb; pdb.set_trace()
            img_path = os.path.join(os.path.join(config.outdir, f"{sample_count}.png"))
            pil_images[0].save(img_path)
            for validation_prompt in validation_prompts_original:
                data_eval = json.loads(validation_prompt)
                if data_eval['prompt'] == data['prompt'][0]:
                    # import pdb; pdb.set_trace()
                    gen_eval_result = evaluate_image(img_path, data_eval)
                    break
            # gen_eval_result = evaluate_image(img_path, data)
            is_correct = gen_eval_result['correct']
            image_result['geneval_correct'] = is_correct
            image_result['tag'] = data['tag']
            print("geneval_correct:",is_correct)


        total_result.append(image_result)
        # import pdb; pdb.set_trace()
            
    with open(file_path, "a") as f:
        for entry in total_result:
...
# image_result = {}
# for sample_count, image in enumerate(total_pil_images):
#     image.save(os.path.join(sample_path, f"{sample_count:05}.png"))
#     # image_result["image_path"] = os.path.join(sample_path, f"{sample_count:05}.png")
#     sample_count += 1
